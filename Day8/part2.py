...
for y in range(1, h - 1):
    for x in range(1, w - 1):
        ...
        for d in range(1, x + 1):
            if trees[y][x] <= trees[y][x-d] or x - d == 0:
                score *= d
                break
        for d in range(1, w - x):
            if trees[y][x] <= trees[y][x+d] or x + d == w - 1:
                score *= d
                break
        for d in range(1, y + 1):
            if trees[y][x] <= trees[y-d][x] or y - d == 0:
                score *= d
                break
        for d in range(1, h - y):
            if trees[y][x] <= trees[y+d][x] or y + d == h - 1:
                score *= d
                break
