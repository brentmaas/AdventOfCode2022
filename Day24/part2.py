...
while target < 3:
    ...
    if target % 2 == 0 and (valleywidth - 1, valleyheight - 1) in states:
        target += 1
        states = [(valleywidth - 1, valleyheight)]
        continue
    if target == 1 and (0, 0) in states:
        target += 1
        states = [(0, -1)]
        continue
    newstates = []
    for x, y in states:
        if y < 0 or y == valleyheight or (not (x, y) in blizzards and not (x, y) in newstates):
        ...
        if y > 0 and not(x, y - 1) in blizzards and not (x, y - 1) in newstates:
            newstates.append((x, y - 1))
        if y >= 0 and y < valleyheight and x + 1 < valleywidth and not (x + 1, y) in blizzards and not (x + 1, y) in newstates:
            newstates.append((x + 1, y))
        if y >= 0 and y < valleyheight and x > 0 and not (x - 1, y) in blizzards and not (x - 1, y) in newstates:
            newstates.append((x - 1, y))
    states = newstates
print(steps)